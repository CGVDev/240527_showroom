...
export const ApartmentModelStill = ({ baseColor, hoveredColor, department, animation, onClick=()=>{}, onDoubleClick=()=>{} })=>{
    ...
    return(
        <a.mesh  
            ...
            position-y={ floor } 
            position-z={ animation.position.to(t=>t * data.position.z) } 
        ...
        >
            <a.meshPhysicalMaterial color={ baseColor } metalness={ 0.01 } roughness={ 0.2 } thickness={0.001} transmission={ 0.5 } transparent={ true } opacity={ animation.transmission }/>
            <Outlines thickness={0.005} color={ 0xFFFFFF } transparent={ true } opacity={0.9}/>
            <sprite scale={ [0.14, 0.05 , 1.0] } position={ [data.position.x > 0.24 ? 0.18 : 0, 0.05, (data.position.x > 0.28) ? 0 : ((data.position.z > 0) ? 0.16 : -0.16) ] }>
                <a.spriteMaterial 
                    attach="material"
                    color={ 0x4AC0B1 }
                >
