...
export const DialRadial = ({ ticks, initialAngle=0, finalAngle, callback, isclockwise=true, value, setUserActivity, iconStart, iconEnd })=>{
    ...
    const valueToCoords = (position)=>{
        ...
        let _real_pos = toRealPosition(position);
        let angle = ((_real_pos * sliceAngle) + initialAngle)%360;
        console.log({angle, position, _real_pos});
    ...
    const { coords, enableCapture, disableCapture, isActive, setCoords, setOrigin } = useMouseCoords({x: 0, y: 0}, valueToCoords(value));
    ...
    useEffect(()=>{
        ...
        // console.log({ y: top + (height/2), x: left + (width/2), width, height, top, left, value})
        // setCoords(valueToCoords(value));
        console.log("INIT...", value)
        setValueToCoords(value);
